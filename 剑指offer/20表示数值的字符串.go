func scanInteger(s string, old int) (bool, int) {
    if old == len(s) {
        return false, old
    }
    new := old
    ...
    return scanUnsignedInteger(s, new)
}

func trimSpace(s string, old int) int {
    for old < len(s) && s[old] == ' ' {
        old++
    }
    return old
...
func scanUnsignedInteger(s string, old int) (bool, int) {
    before := old 
    new := before
    for new < len(s) && s[new] >= '0' && s[new] <= '9' {
        new++
    }
    return new > before, new
}
...
func isNumber(s string) bool {
    ...
    p := trimSpace(s, 0)
    res, p := scanInteger(s, p)

    // fmt.Println("p1", p)
    if p < len(s) && s[p] == '.' {
        p++
        sub, nextp := scanUnsignedInteger(s, p)
        // fmt.Println("p2", p)
        res = res || sub
        p = nextp
    }
...
func isNumber_SM(s string) bool {
